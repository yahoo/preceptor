...
var MochaClient = AbstractClient.extend(

	{
		...
		_runMocha: function (options, success, failure) {
			...
			if (options.require) {
				options.require.forEach(function (requireFile) {
					utils.require(path.resolve(requireFile));
				});
			}
			...
			if (options.functions) {
				mocha.loadFiles = function () {
					...
					// Load the functions
					options.functions.forEach(function (currentFn, index) {

						var fileName = currentFn.name || 'function-' + index;

						mocha.suite.emit('pre-require', global, fileName, mocha);
						mocha.suite.emit('require', (_.isString(currentFn) ? utils.require(currentFn) : currentFn()), fileName, mocha);
