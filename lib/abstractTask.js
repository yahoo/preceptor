...
var AbstractTask = Base.extend(
	...
	function (config, coverageCollector, reportManager, plugins, options) {
		...
		logger.trace("Config:", config.toObject());
		...
		logger.debug("Merged options:", this._options);
...

	{
		...
		getTaskPlugin: function (name) {
			...
			var result,
				task = this.getPlugins().task || {};

			name = name.toLowerCase();
			result = task[name];

			if (!result) {
				throw new Error('The Task-Plugin with name "' + name + '" does not exist.');
			}
		...
		run: function (parentId) {
			...
			if (this.isActive()) {
				...
				logger.debug('Skip task as it is inactive.');
