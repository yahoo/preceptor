...
var PreceptorManager = Base.extend(
...

	{
		...
		initialize: function () {
			...
			logger.trace("Create config");
			this._configuration = new Config(this._options.configuration);
			if (this.getConfig().isVerbose() && !log.getLevel()) {
				log.setLevel('DEBUG');
			}
			log.flush();

			// Set-up the report-manager
			logger.trace("Create report manager", this.getConfig().getReportManager());
			this._reportManager = new ReportManager(this.getConfig().getReportManager());

			// Load custom plugins
			plugins = this.getConfig().getPlugins();
			if (plugins.length > 0) {
				logger.info("Initialize plugins", plugins);
				plugins.forEach(function (pluginPath) {

					logger.debug("Load plugin:", pluginPath);
					var Plugin = utils.require(pluginPath);

					if (Plugin.loader && _.isFunction(Plugin.loader)) {
						logger.trace("Execute plugin loader");
						Plugin.loader(this);
					} else {
		...
		_initializeTaskDecoratorRegistry: function () {
			...
			logger.debug("Initialize task decorator registry");
			defaultElements.forEach(function (entry) {
				logger.trace("Load task decorator:", entry.name);
				entry.path = path.join(__dirname, 'taskDecorator', entry.fileName);
				entry.fn = utils.require(entry.path);
			}, this);
			logger.trace("Done initializing task decorator registry");
		...
		_initializeClientDecoratorRegistry: function () {
			...
			logger.debug("Initialize client-decorator registry");
			defaultElements.forEach(function (entry) {
				logger.trace("Gather client decorator:", entry.name);
				entry.path = path.join(__dirname, 'clientDecorator', entry.fileName);
			}, this);
			logger.trace("Done initializing client-decorator registry");
		...
		_initializeTaskRegistry: function () {
			...
			logger.debug("Initialize task registry");
			defaultElements.forEach(function (entry) {
				logger.trace("Load task:", entry.name);
				entry.path = path.join(__dirname, 'task', entry.fileName);
				entry.fn = utils.require(entry.path);
			}, this);
			logger.trace("Done initializing task registry");
		...
		hasTaskDecorator: function (name) {
			return !!this.getTaskDecoratorList()[name.toLowerCase()];
		},
		...
		getTaskDecorator: function (name) {
			var lowerCaseName = name.toLowerCase(),
				result = this.getTaskDecoratorList()[lowerCaseName];

			if (!result) {
				throw new Error("Could not find task-decorator: " + lowerCaseName);
			}
		...
		hasClientDecorator: function (name) {
			return !!this.getClientDecoratorList()[name.toLowerCase()];
		},
		...
		getClientDecorator: function (name) {
			var lowerCaseName = name.toLowerCase(),
				result = this.getClientDecoratorList()[lowerCaseName];

			if (!result) {
				throw new Error("Could not find client-decorator: " + lowerCaseName);
			}
		...
		hasTask: function (name) {
			return !!this.getTaskList()[name.toLowerCase()];
		},
		...
		getTask: function (name) {
			var lowerCaseName = name.toLowerCase(),
				result = this.getTaskList()[lowerCaseName];

			if (!result) {
				throw new Error("Could not find task-type: " + lowerCaseName);
			}
		...
		run: function () {
			...
			logger.debug('Shared task options:', this.getSharedTaskOptions());
			logger.debug('Configuration:', config.toObject());
			logger.debug('Tasks:', this.getTasks());

			logger.trace('Add event reporter');
			eventReporter = reportManager.addReporter("Event"); // Needed to forward it to the client-decorator

			logger.trace("Add reporter:", config.getReporter());
			reportManager.addReporterRange(config.getReporter());

			logger.trace("Add listener:", config.getListener());
			reportManager.addListenerRange(config.getListener());

			// Output events from reporter
			logger.trace('Hook-up event reporter');
			eventReporter.on('message', function (areaType, messageType, params) {
				logger.debug('Event reporter - message received: [' + areaType + '] ' + messageType, params);
			});
			...
			promise = promise.then(function () {
				...
				return task.run(undefined); // The top-most task has parent-id 'undefined'
		...
		_writeCoverage: function (coverageConfiguration) {
			...
			logger.trace('Creating coverage report folder:', reportingDir);
			mkdirp.sync(reportingDir);

			if (reports.indexOf('file') !== -1) {

				// Write JSON file
				logger.info('Writing JSON coverage report:', coverageFile);
			...
			// Write report
			logger.info('Writing coverage reports');
