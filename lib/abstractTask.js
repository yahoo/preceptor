...
var AbstractTask = Base.extend(
	...
	function (config, coverageCollector, reportManager, plugins, options) {
		...
		this._options = utils.deepExtend({}, [defaultTask, options || {}]);
...

	{
		...
		validate: function () {
			...
			if (!_.isObject(this.getDecorator())) {
				throw new Error('The "decorator" parameter is not an object.');
			}
		...
		getDecorator: function () {
			return this.getOptions().decorator;
		},
		...
		getTaskPlugin: function (name) {
			return this.getPlugins().task[name];
		},
