		...
		runClient: function (parentId, clientPath) {
			...
			return new Promise(function (resolve, reject) {
				...
				if (!fs.existsSync(clientPath) && !fs.existsSync(clientPath + '.js')) {
					throw new Error("The client-plugin does not exist: " + clientPath);
				}
				...
				if (this.inDebug()) {
					...
					ClientClass = utils.require(clientPath);
					...
					tempPath = path.resolve(path.join(__dirname, 'client.js'));

					logger.debug('Fork task-client:', clientPath, 'with client-path:', tempPath);
					if (!fs.existsSync(tempPath) && !fs.existsSync(tempPath + '.js')) {
						throw new Error("The abstract-client does not exist: " + tempPath);
					}

					clientInstance = childProcess.fork(tempPath, { silent: true });
