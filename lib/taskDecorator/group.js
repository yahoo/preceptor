...
var GroupTaskDecorator = AbstractTaskDecorator.extend(

	{
		...
		run: function (taskOptions, taskIndex) {
			...
			logger.trace("Run group task-decorator");
		...
		_processWhenObject: function (taskOptions) {
			...
			// The type-check is there so that this function doesn't wrap tasks from _processWhenArray
			if (_.isObject(taskOptions) && !_.isArray(taskOptions) && !taskOptions.type) {

				newList = [];

				_.keys(taskOptions).forEach(function (key) {
					newList.push({
						"type": "group",
						"name": key,
						"title": key,
